...
import { CategoryStrip } from './CategoryLabel';
...
interface TaskSidebarProps {
  ...
  width: number;
  deliverableColors?: Map<string, string>;
}
      ...
      {deliverables.map((deliverable) => {
        ...
        return (
          <div key={deliverable.id}>
            {/* Deliverable Header */}
            <div
              className="relative flex items-center pl-3 pr-4 py-2 border-b border-slate-200 bg-slate-100 hover:bg-slate-200 cursor-pointer"
              style={{ height: ROW_HEIGHT }}
              onClick={() => toggleDeliverable(deliverable.id)}
            >
              {/* Color strip */}
              <CategoryStrip colour={categoryColor} height={ROW_HEIGHT} isHeader />
              
              <button className="mr-2 text-slate-600 ml-1">
            ...
            {!isCollapsed && deliverable.tasks.map((task) => {
              ...
              return (
                <div
                  key={task.id}
                  className={`
                    relative flex items-center pl-3 pr-4 py-2 border-b border-slate-100 hover:bg-slate-50 cursor-pointer transition-colors
                    ${isSelected ? 'bg-blue-50' : ''}
                  `}
                  style={{ height: ROW_HEIGHT }}
                  onClick={() => onTaskClick(task.id)}
                >
                  {/* Color strip (lighter for tasks) */}
                  <CategoryStrip colour={categoryColor} height={ROW_HEIGHT} />
                  
                  <input
                    type="checkbox"
                    className="mr-3 ml-1 rounded border-slate-300"
                    checked={isSelected}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-500 flex items-center gap-1.5">
                      {task.resources.length > 0 && (
                        <>
                          <span className="truncate max-w-[100px]">{task.resources[0].resourceName}</span>
                          <span>•</span>
                        </>
                      )}
                      ...
                      {task.costEstimated > 0 && (
                        <>
                          <span>•</span>
                          <span>${task.costEstimated.toLocaleString()}</span>
                        </>
                      )}
                    </div>
                  </div>
